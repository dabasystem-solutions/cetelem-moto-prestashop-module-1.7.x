...
class CetelemCallbackModuleFrontController extends ModuleFrontController {
  ...
  public function postProcess() {
    ...
    if (Tools::isSubmit('IdTransaccion') AND Tools::isSubmit('codResultado')) {
      ...
      $string = Tools::getValue('IdTransaccion') . '|' . Tools::getValue('NSolicitud') . '|' . Tools::getValue('codResultado');
  ...
  public function valdiationWithoutURL() {
    ...
    if ($cart->id_customer == 0 || $cart->id_address_delivery == 0 || $cart->id_address_invoice == 0 || !$this->module->active) {
      PrestaShopLogger::addLog('Cetelem::CallBack - Cart not completed', 1, null, 'Transaction Id Cart', (int) $cart->id, true);
    }
    ...
    if (!$authorized)
      PrestaShopLogger::addLog('Cetelem::CallBack - This payment method is not available', 1, null, 'Transaction Id Cart', (int) $cart->id, true);

    $customer = new Customer($cart->id_customer);
    if (!Validate::isLoadedObject($customer))
      PrestaShopLogger::addLog('Cetelem::CallBack - Customer Object Not Valid', 1, null, 'Transaction Id Cart', (int) $cart->id, true);
    ...
    if (file_exists($filename)) {
      if ($string = Tools::file_get_contents($filename)) {
        ...
        if (empty($CodResultado) AND empty($IdTransaccion))
          PrestaShopLogger::addLog('Cetelem::CallBack - Transaction file without Id Transaction or Code Result', 1, null, 'Transaction Id Cart', (int) $cart->id, true);
        ...
        if ($CodResultado == '00') {
          ...
          if (!Validate::isLoadedObject($order_state)) {
            PrestaShopLogger::addLog('Cetelem::CallBack - The new order status is invalid.', 1, null, 'Transaction Id Cart', (int) $cart->id, true);
          } else {
...
//              }
      ...
      PrestaShopLogger::addLog('Cetelem::CallBack - File from Cetelem Transaction not exists.', 1, null, 'Transaction Id Cart', (int) $cart->id, true);
    }
  }
      ...
      if ($string = Tools::file_get_contents($filename)) {
        ...
        if ($CodResultado == '00') {
          if (Configuration::getGlobalValue('PS_OS_CETELEM_APPROVED') != (int) $c_order_state->id) {
            /* Change order status, add a new entry in order history and send an e-mail to the customer if needed */
            $this->module->validateOrder($cart->id, Configuration::getGlobalValue('PS_OS_CETELEM_PREAPPROVED'), $total, $this->module->name, '', $mailVars, (int) $currency->id, false, $customer->secure_key);
          }
        } elseif ($CodResultado == '50') {
          /* Change order status, add a new entry in order history and send an e-mail to the customer if needed */
          $this->module->validateOrder($cart->id, Configuration::getGlobalValue('PS_OS_CETELEM_APPROVED'), $total, $this->module->name, '', $mailVars, (int) $currency->id, false, $customer->secure_key);
        } elseif ($CodResultado == '99' || $CodResultado == '51') {
          $order_state = new OrderState(Configuration::getGlobalValue('PS_OS_CETELEM_DENIED'));
          $this->module->validateOrder($cart->id, Configuration::getGlobalValue('PS_OS_CETELEM_DENIED'), $total, $this->module->name, '', $mailVars, (int) $currency->id, false, $customer->secure_key);
        } else {
          /* Change order status, add a new entry in order history and send an e-mail to the customer if needed */
          $this->module->validateOrder($cart->id, Configuration::getGlobalValue('PS_OS_CETELEM_PREAPPROVED'), $total, $this->module->name, '', $mailVars, (int) $currency->id, false, $customer->secure_key);
        }
