...
class Cetelem extends PaymentModule implements WidgetInterface {
  ...
  const CETELEM_URL_TEST_CONNECTION = 'https://test.cetelem.es/eCommerceLite/moto/configuracion.htm';
  const CETELEM_URL_CONNECTION = 'https://www.cetelem.es/eCommerceLite/moto/configuracion.htm';
  ...
  public function __construct() {
    ...
    $this->displayName = $this->l('Cetelem / ecreditmoto');
  ...
  protected function createCetelemPrepprovedState() {
    ...
    if (!Configuration::getGlobalValue('PS_OS_CETELEM_PREAPPROVED') || !$tmp_o_state->name) {
      ...
      return $this->createCetelemState($names, $templates, 'PS_OS_CETELEM_PREAPPROVED', '#ffff00', 'cetelem-preapproved');
    } else {
      $tmp_o_state = new OrderState((int) Configuration::getGlobalValue('PS_OS_CETELEM_PREAPPROVED'));
  ...
  public function createCetelemStandByState() {
    ...
    if (!Configuration::getGlobalValue('PS_OS_CETELEM_STANDBY') || !$tmp_o_state->name) {
      ...
      return $this->createCetelemState($names, $templates, 'PS_OS_CETELEM_STANDBY', '#4169E1', 'cetelem-preapproved');
    } else {
      $tmp_o_state = new OrderState((int) Configuration::getGlobalValue('PS_OS_CETELEM_STANDBY'));
  ...
  protected function createCetelemApprovedState() {
    ...
    if (!Configuration::getGlobalValue('PS_OS_CETELEM_APPROVED') || !$tmp_o_state->name) {
      ...
      return $this->createCetelemState($names, $templates, 'PS_OS_CETELEM_APPROVED', '#32CD32', 'cetelem-approved', true);
    } else {
      $tmp_o_state = new OrderState((int) Configuration::getGlobalValue('PS_OS_CETELEM_APPROVED'));
  ...
  protected function createCetelemDeniedState() {
    ...
    if (!Configuration::getGlobalValue('PS_OS_CETELEM_DENIED') || !$tmp_o_state->name) {
      ...
      return $this->createCetelemState($names, $templates, 'PS_OS_CETELEM_DENIED', '#DC143C', 'cetelem-denied');
    } else {
      $tmp_o_state = new OrderState((int) Configuration::getGlobalValue('PS_OS_CETELEM_DENIED'));
   ...
   * @todo
   */
  protected function createCetelemState($name_translations, $template_translations, $config_variable, $state_color, $icon_name, $logable_false = false) {
    //create orders
    ...
    if ($OrderState->add()) {
      ...
      copy(_PS_MODULE_DIR_ . $this->name . '/views/img/' . $icon_name . '.gif', _PS_IMG_DIR_ . 'os/' . $OrderState->id . '.gif');
    } else
      return false;
  ...
  public function postProcess() {
    ...
    if (Tools::isSubmit('submitCetelemConnectData')) {
      ...
      foreach (Language::getLanguages() as $lang) {
        $cetelemInfo[$lang['id_lang']] = htmlspecialchars(Tools::getValue('CETELEM_INFO_CALC_TEXT_' . $lang['id_lang']));
      }
  ...
  public function getContent() {
    ...
    if (version_compare(_PS_VERSION_, '1.6', '<'))
      $this->output .= $this->context->smarty->fetch($this->local_path . 'views/templates/admin/information15.tpl');
    else
    ...
    $this->fields_form[0]['form'] = array(
        ...
        'input' => array(
            ...
            array(
                ...
                'lang' => false,
                'col' => 4,
                $msg => $this->l('Indicate in this field your client code provided by Cetelem. In case of not knowing what contact Cetelem.')
            ),
        ),
        'submit' => array(
  ...
  public function displayFormConfig() {
    ...
    $this->fields_form[0]['form'] = array(
        ...
        'input' => array(
            ...
            array(
                ...
                $msg => $this->l('IMPORTANT NOTE: if you select an amount with less than 150 €,all orders with less than this amount will be declined '),
                'required' => false,
                'lang' => false,
                'suffix' => '€',
                'col' => 4,
            ...
            array(
                ...
                $msg => $this->l('Keep this option enabled to increase the average size of your cart and conversion rates'),
                'required' => false,
                'is_bool' => true,
                'class' => 't',
                'values' => array(
    ...
    foreach (Language::getLanguages() as $lang) {
      $helper->fields_value['CETELEM_INFO_CALC_TEXT'][$lang['id_lang']] = htmlspecialchars_decode(Configuration::get('CETELEM_INFO_CALC_TEXT', $lang['id_lang']));
    }
  ...
  public function hookHeader() {
    ...
    if ((!isset($this->context->controller->php_self) || ($this->context->controller->php_self != 'product' && $this->context->controller->php_self != 'cms') || (!Configuration::get('CETELEM_DISPLAY_CALC') && $this->context->controller->php_self != 'cms')) && get_class($this->context->controller) != 'CetelemCalculatorModuleFrontController')
      return;
    $tmp_product = false;

    if (Tools::getValue('id_product')) {
      if (Product::getPriceStatic(Tools::getValue('id_product'), true) >= (float) Configuration::get('CETELEM_MIN_AMOUNT')) {
        if (version_compare(_PS_VERSION_, '1.6', '<'))
        ...
        if (Tools::getValue('id_product')) {
          ...
          $tax_inc = Configuration::get('PS_TAX');
          ...
          foreach ($cetelemCombinations as $cetelemCombination) {
            $cetelemCombPrices[$cetelemCombination['id']] = $tmp_product->getPrice($tax_inc, $cetelemCombination['id'], 6);
          }
          $this->smarty->assign(array(
              'productPriceC' => $cetelemPrice, 'cetelemCombPrices' => $cetelemCombPrices));
        }
      } else {
        return;
      }
    ...
    $center_code = Configuration::get('CETELEM_CLIENT_ID');
    $calc_type = $this->getCalcTypeScript(Configuration::get('CETELEM_CALC_TYPE'));
  ...
  public function hookPaymentOptions($params) {
    if ($this->context->cart->getOrderTotal() < Configuration::get('CETELEM_MIN_AMOUNT'))
      return;

    //we check if there is a motorcycle in the cart and if is the case, we dont permit to finance it with Cetelem and will show a message saying that to finance it it will need to be only 1 product in the cart.
    $products_ = $params['cart']->getProducts();
    $hasbike = false;
    $bike_material = false;
    $registration_date = false;
    $bike_brand_code = false;
    foreach ($products_ as $product_) {
        if (count($product_['features'])) {
            foreach ($product_['features'] as $feature) {
                $feature_name = new Feature((int)$feature['id_feature']);
                // var_dump($feature_name->name);
                foreach ($feature_name->name as $fname) {
                    /*if ($fname == 'isbike') {
                        $hasbike = true;
                        //ddd($feature['id_feature_value']);
                        /*$value_lang = FeatureValue::getFeatureValueLang((int)$feature['id_feature_value']);
                        if (count($value_lang)) {
                            ddd($value_lang[0]['value']);
                        }*/
                        //ddd(FeatureValue::getFeatureValueLang((int)$feature['id_feature_value']));
                    /*} else*/
                    if ($fname == 'bikematerial') {
                        $hasbike = true;
                        $value_lang = FeatureValue::getFeatureValueLang((int)$feature['id_feature_value']);
                        if (count($value_lang)) {
                            //ddd($value_lang[0]['value']);
                            if ($value_lang[0]['value'] != '') {
                                $bike_material = $value_lang[0]['value'];
                            }
                        }
                    } elseif ($fname == 'bikebrandcode') {
                        $value_lang = FeatureValue::getFeatureValueLang((int)$feature['id_feature_value']);
                        if (count($value_lang)) {
                            //ddd($value_lang[0]['value']);
                            if ($value_lang[0]['value'] != '') {
                                $bike_brand_code = $value_lang[0]['value'];
                            }
                        }
                    } elseif ($fname == 'registrationdate') {
                        $value_lang = FeatureValue::getFeatureValueLang((int)$feature['id_feature_value']);
                        if (count($value_lang)) {
                            //ddd($value_lang[0]['value']);
                            if ($value_lang[0]['value'] != '') {
                                $registration_date = $value_lang[0]['value'];
                            }
                        }
                    }
                }
    ...
    //ddd($hasbike);
    if ($hasbike && count($products_) > 1) {
        //ddd('You cant put more than 1 product in your cart when you are upurchasing a bike if you want to finance it');
        $result = $this->display(__FILE__, 'views/templates/hook/payment_not_allowed.tpl');
        return $result;
    }
    ...
    $loaded_address = Validate::isLoadedObject($address);
    $fields = array('cart' => $cart, 'transact_id' => $transact_id, 'amount' => $amount, 'timestamp1' => $timestamp1, 'loaded_customer' => $loaded_customer, 'gender' => $gender, 'address' => $address, 'birthday' => $birthday, 'addressText' => $addressText, 'loaded_address' => $loaded_address, 'id_language' => $id_language);
    /* or ask for confirmation */
    /* ecredit moto */
    $fields['bike_material'] = $bike_material;
    $fields['bike_brand_code'] = $bike_brand_code;
    ...
    if ((Tools::strtoupper($this->context->language->iso_code) == 'ES') && (Configuration::get('CETELEM_LEGAL_NOM_PAGO') != '')) {
      $this->context->smarty->assign(array(
  ...
  public function getGPaymentOption($fields) {
    //new calculator
    ...
    $conex_url = (Configuration::get('CETELEM_ENV')) ? self::CETELEM_URL_CONNECTION : self::CETELEM_URL_TEST_CONNECTION;
    $form_next_url = Configuration::get('CETELEM_ORDER_CREATION') ? $this->context->link->getModuleLink($this->name, 'payment2', array(), true) : $conex_url;
...
//    }
      ...
      $externalOption->setCallToActionText($payment_text)
              ...
              ->setInputs([
                  ...
                  'CodigoMarca' => [
                      'name' => 'CodigoMarca',
                      'type' => 'hidden',
                      'value' => $fields['bike_brand_code'] ? $fields['bike_brand_code'] : '',
                  ]
                  ,
                  'FechaMatriculacion' => [
                      'name' => 'FechaMatriculacion',
                      'type' => 'hidden',
                      'value' => $fields['registration_date'] ? $fields['registration_date'] : '',
                  ]
                  ,
                  /*'CodProducto' => [
                      'name' => 'CodProducto',
  ...
  public function hookDisplayRightColumnProduct($params) {
    if (!Configuration::get('CETELEM_CALC_POSITION')) {
      return $this->calculatorHookRenderer($params);
    } else {
      return;
    }
  }
  ...
  public function calculatorHookRenderer($params) {
    if (Configuration::get('CETELEM_DISPLAY_CALC')) {
      ...
      $this->smarty->assign(array(
          'customer_group_without_tax_cetelem' => Group::getPriceDisplayMethod($this->context->customer->id_default_group),
          'group_reduction_cetelem' => $group_reduction,
          'productObjCetelem' => $product,
          'ecotaxTax_rate_cetelem' => (float) Tax::getProductEcotaxRate($this->context->cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')}),
      ));
      ...
      $this->smarty->assign(array(
          ...
          'info_text' => htmlspecialchars_decode(Configuration::get('CETELEM_INFO_CALC_TEXT', $this->context->language->id)),
          'amount' => $amount,
      ...
      if (Configuration::get('CETELEM_DISPLAY_CALC') && Configuration::get('CETELEM_MIN_AMOUNT') <= $product->getPrice(true)) {
  ...
  protected function displayCalculator() {
    if (version_compare(_PS_VERSION_, '1.6', '<'))
      return $this->display(__FILE__, 'views/templates/hook/product-calc15.tpl');
    else
  ...
  protected function sendCetelemEmail($template, $id_order, $order_state) {
    ...
    Mail::Send(
            $customer->id_lang, $template, $order_state->name, $data, $customer->email, $customer->firstname . ' ' . $customer->lastname, null, null, null, null, _PS_MODULE_DIR_ . $this->name . '/mails/', false, (int) $order->id_shop
    );
  }
  ...
  public function getCalcTypeScript($calc_type) {
    ...
    switch ($calc_type) {
      case 0:
        //combo1
        return '/eCommerceCalculadora/resources/js/eCalculadoraCetelemCombo.js';
        break;
      case 1:
        //combo2
        return '/eCommerceCalculadora/resources/js/eCalculadoraCetelemComboModelo2.js';
        break;
      case 2:
        //slider
        return '/eCommerceCalculadora/resources/js/eCalculadoraCetelemSlider.js';
        break;
      default:
        //combo1
        return '/eCommerceCalculadora/resources/js/eCalculadoraCetelemCombo.js';
  ...
  public function getTemplateVars() {
    ...
    $text_payment = '';
    if ((Tools::strtoupper($this->context->language->iso_code) == 'ES') && (Configuration::get('CETELEM_LEGAL_CHECKOUT') != '')) {
      $text_payment = Configuration::get('CETELEM_LEGAL_CHECKOUT');
    }

    $total = $this->trans(
            '%amount% (tax incl.)', array(
  ...
  private function updateaproved_state() {
    //updateaproved_state
  ...
  {
    return $this->calculatorHookRenderer(null);
  }
  ...
  public function hasSpecialMaterial()
  {
    ...
    if (Configuration::get('CETELEM_SPEC_MATERIAL')) {
      if ($this->context->cart->getOrderTotal() >= 3000) {
        ...
        $i = 0;
        foreach ($this->context->cart->getProducts() as $key => $product_) {
