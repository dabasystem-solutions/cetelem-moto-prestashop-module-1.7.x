...
<div class="row">
    <div class="col-xs-12">
        ...
        {if Configuration::get('CETELEM_ORDER_CREATION')}
            <a id="confirm_cetelem_order" href="ajaxCetelem.createOrder();" title="{l s='Buy with a Cetelem credit' mod='cetelem'}">
        {else}
            <a href="javascript:$('#cetelem_form').submit();" href="ajaxCetelem.createOrder();" title="{l s='Buy with a Cetelem credit' mod='cetelem'}">
        {/if}
            ...
            {/if}
            {if isset($text_payment) && $mode == 'G'}<span id="extraInfoPayment">{$text_payment|escape:'html':'UTF-8'}</span>{/if}
            </a>
        </p>
